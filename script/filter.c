...
void flow_fusion(float dT,float fx,float fy,s32 flow_height,u8 pos_hold)
{
	...
	if( pos_hold == 1)   //in pose hold mode
	{
		...
		LPF_1_(3.0f,dT,gyro_x,gyro_lpf_x);	// 使得光流输出的相位与陀螺仪相位一致
			
		fx_gyro_fix = ((fx  + LIMIT(((gyro_lpf_y)/57.3f),-flow_t1,flow_t1)) *10 *use_height ) ;  //rotation compensation
		fy_gyro_fix = ((fy  - LIMIT(((gyro_lpf_x)/57.3f),-flow_t2,flow_t2)) *10 *use_height ) ; //用陀螺仪补偿后的光流角速度？使得飞机在只有旋转没有平移时光流最终输出为零
		
		vec_3d_transition(&imu_data.z_vec, &imu_data.a_acc, &heading_coordinate_acc);   //机体坐标系下加速度到水平机体航向坐标系(将机体水平放置，没有偏航角，
		                                                                                 //保持加速度与光流没有偏航方向的影响)
					
		f1_fx.out += ((s32)heading_coordinate_acc.x/10 *10 ) *dT;   //integrated  acceleration for speed 当前认为的速度（通过imu的加速度积分得到的）
		...
		heading_coordinate_speed_fus.x  = f1_fx.out;   //融合速度，将融合后的速度重新认为是当前速度
		...
		f_out_y = heading_coordinate_speed_fus.y + 0.1f *heading_coordinate_speed_err_i.y; //由于加速度计积分有漂移，
																// 因此可能造成融合的速度在悬停或运动中会漂移真实值，而光流的计算是没有漂移及累积误差的，
																// 因此以光流数据做参考，对f1_fx.out做一个二次修正，得到最终融合速度输出f_out_x
